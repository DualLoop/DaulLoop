...
def main():
    ...
    commands.add_parser("profiles", help="list all nine supported RQ1/RQ2 profiles")
    p = commands.add_parser("prepare", help="write a new campaign without any model call")
    p.add_argument("--rq", type=int, choices=(1, 2, 3), required=True)
    ...
    p.add_argument("--collection", choices=("a", "b"), default="a")
    ...
    if args.command == "prepare":
        if args.profile == "all" and (args.rq == 3 or args.out):
            parser.error("--profile all is only for RQ1/RQ2, without --out")
