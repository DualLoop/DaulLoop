...
RQ3 uses the replay image to generate and verify candidate instances offline before
sending selected images to the evaluator API. No GPU is needed locally.

Docker builds and coding-agent setup download dependencies. Agent containers have public
network access to reach their provider. Before protected verification, the controller
replaces that container with one that has no network access. Protected tests are
uploaded after the agent phase. Credentials are copied temporarily with mode 0600,
removed after use, and checked absent in the replacement container. Candidate code is
...
included only in the temporary smoke-test files; normal discovery episodes do not
receive it.

Select an experiment with `--rq 1`, `--rq 2`, or `--rq 3`. RQ3 uses collection A by
default; add `--collection b` to check the other starting collection. Use a different
`--out` directory for each check. These commands test input preparation and protected
verification; they do not call a coding model or an RQ3 evaluator.
...
## Check the starting collections
...
protected verification during a campaign.
...
after passing protected checks and the controller's retention criteria. This does not
