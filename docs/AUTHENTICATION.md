...
temporary private copy; the whole home directory is never mounted into the builder.
...
RQ3 additionally uses an **OpenRouter key for evaluator calls**, even with a Codex or
Claude builder. Its confirmation must equal the exact campaign ID:
