# RQ2 — Controllability: image-support-steered discovery

**Paper:** §4.4 (setup), §5.2 (results).

RQ2 tests whether the agent can control which regions of an image its inverse program
depends on. It extends RQ1 by requesting a particular spatial scale and shape for those
regions.

## Inputs and controller behavior

Each episode receives a fixed profile card and the three shared implementation examples,
plus a requested support scale (`focal`, `regional`, `distributed`) and shape
(`compact`, `pathlike`, `multipart`). The controller updates its category record and
selects later requests from prior measurements.

Support is measured by masking image regions and checking whether the inverse answer
changes or the inverse program fails. This is inverse-program sensitivity, not human
attention. A difference between the requested and measured categories does not by itself
reject a world. Protected verification and the retention criteria still apply. The
internal implementation uses `quality_diversity` names for this category record.
...
After building the Docker images, run the fixed-example check for RQ2:
...
git commit -m "Freeze RQ2 example campaign"
...
git commit -m "Freeze nine RQ2 profile campaigns"
```
