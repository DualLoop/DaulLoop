# Render the three shared implementation examples

**Paper §4.2:** the shared examples teach agents how a question world is implemented.
They cover two-circle contact, angle acuteness, and counting with distractors. They are
distinct from the five text-only directions in each discovery profile.
...
This renders 12 scenes per world with random seed 101, computes the expected answers and
...
manifest records scene specifications, exact question text, and expected answers. To
...
During discovery, agents receive only the historical five-file snapshots: `DESIGN.md`,
