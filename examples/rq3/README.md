# RQ3 — Evaluator guidance: model-feedback discovery

**Paper:** §4.5–4.6 (setup), §5.4 (results).

RQ3 starts from ten existing worlds selected for difficulty on the evaluator models.
Later coding-agent episodes use evaluator errors to propose new worlds.

## Inputs and controller behavior

Choose collection `a` or `b`. Each contains ten starting world implementations and tests
with fixed feedback summaries. RQ3 also receives the three shared examples and a card
describing the starting worlds and difficulty objective; it does not use the nine
semantic profiles.

After a new candidate passes protected checks, the controller renders ten instances
using fixed random seeds and sends five selected instances to each of three evaluator
models. A world qualifies as difficult for the evaluator panel ("panel-hard") when at
least two evaluators answer at most three of five correctly, with all 15 responses
present. Hardness does not replace protected verification. Later episodes receive
summaries of completed evaluations.

The initial feedback JSON includes gold answers, predictions, and correctness. There are
two detailed samples per starting world, while aggregate scores use five instances per
world. No initial image files are supplied. Starting-world answer visibility is separate
from protected checking of a new submission.
...
After building the Docker images, run the fixed-example check for RQ3:
...
This checks the episode inputs, container replacement, and protected verification. It
...
its host prerequisites and provider confirmation variables. RQ3 additionally requires
`OPENROUTER_API_KEY` for evaluator calls, even when using a Codex or Claude builder. Set
the feedback confirmation to this campaign ID:
...
git commit -m "Freeze RQ3 example campaign"
...
For a six-hour campaign on the other starting collection:
...
two collections; this release supplies three example presets and the episodic-sequential
0.7 controller variant. It does not reproduce every historical RQ3 variant. See [release
differences](../../docs/RELEASE_NOTES.md).
