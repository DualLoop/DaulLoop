# RQ1 — Construction: profile-conditioned discovery

**Paper:** §4.3 (setup), §5.1 (results).

RQ1 tests whether a coding agent can repeatedly invent visual question worlds that pass
protected verification. One campaign uses one fixed visual-computation profile.

## Inputs and controller behavior

Each episode receives the same three shared implementation examples and the same profile
card, including all five text-only question directions. Accepted world code and the most
recent rejected submission and its report change as the campaign progresses. The
question directions do not come with profile-specific images or implementations.
...
`reveal_declared_transform`. See the [exact profile
cards](../../foundry/discovery_profiles/paper-semantic/v0.3/).
...
After building the Docker images, run the fixed-example check for RQ1:
...
git commit -m "Freeze RQ1 example campaign"
...
git commit -m "Freeze nine RQ1 profile campaigns"
```
